...
def main():
    parser = argparse.ArgumentParser(description='Optimized Fast Training: ViT TextileNet')
    ...
    if args.resume:
        ckpt_path = f'./{args.partition}_results/checkpoint_vit/ckpt.pth'
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            progress_bar(i, len(train_loader), f'Loss: {running_loss/(i+1):.3f} | Acc: {100.*correct/total:.2f}%')
        ...
        with torch.no_grad():
            for i, (inputs, targets) in enumerate(test_loader):
                ...
                total += targets.size(0)
                correct += preds.eq(targets).sum().item()
                progress_bar(i, len(test_loader), f'Val Acc: {100.*correct/total:.2f}%')

        val_acc = 100.*correct/total
        if val_acc > best_acc:
            save_dir = f'./{args.partition}_results/checkpoint_vit'
            os.makedirs(save_dir, exist_ok=True)
            torch.save({'model': model.state_dict(), 'acc': val_acc, 'epoch': epoch},
