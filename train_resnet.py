...
# Argument parser
parser = argparse.ArgumentParser(description='PyTorch TextileNet ResNet18 Training (AMP + EarlyStopping)')
...
train_loader = DataLoader(
    ...
    prefetch_factor=2, persistent_workers=True, collate_fn=my_collate
)
...
best_acc = 0
if args.resume:
    ckpt_dir = f"./{DATA}_results/checkpoint_res"
...
def train_one_epoch():
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        progress_bar(i, len(train_loader),
                     f"Train Loss: {running_loss/(i+1):.3f} | Acc: {100.*correct/total:.2f}%")
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(test_loader):
            ...
            progress_bar(i, len(test_loader),
                         f"Val Acc so far: {100.*correct/total:.2f}%")
    acc = 100.*correct/total
    if acc > best_acc:
        os.makedirs(f"./{DATA}_results/checkpoint_res", exist_ok=True)
        torch.save({'model': model.state_dict(), 'acc': acc, 'epoch': epoch},
                   f"./{DATA}_results/checkpoint_res/ckpt.pth")
        best_acc = acc
    return acc
